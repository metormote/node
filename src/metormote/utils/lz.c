...
uint32_t LZ_Compress( unsigned char *in, unsigned char *out,
    uint32_t insize )
{
    unsigned char startblock, marker, symbol;
    ...
    //Use fixed markers to save memory
    startblock='\x02';
    marker='\x1B';
    
    
    /* Remember the marker symbols for the decoder */
    out[ 0 ] = startblock;
    out[ 1 ] = marker;
    ...
    while( inpos < insize )
    {
        ...
        else if( in[ inpos ] == startblock )
        {
            out[ outpos ++ ] = startblock;
            out[ outpos ++ ] = 0;
        }
        else
        {
    ...
    uint32_t insize )
{
    ...
    {
        symbol = in[ inpos ++ ];
        if( symbol == startblock ) {
            /* We had a marker byte */
            if( in[ inpos ] == 0 )
            {
               /* It was a single occurrence of the startblock byte */
                out[ outpos ++ ] = startblock;
                ++ inpos;
            }
            else
